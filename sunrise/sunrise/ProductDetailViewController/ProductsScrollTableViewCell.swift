//
//  ProductsScrollTableViewCell.swift
//  sunrise
//
...
protocol ProductsScrollTableViewCellDelegate {
    
    func productSelectedWithProductInfo(productInfo : [String : AnyObject])
}

class ProductsScrollTableViewCell: UITableViewCell {
    ...
    var delegate : ProductsScrollTableViewCellDelegate?
    ...
    func setDataWith(model : [[String : AnyObject]])    {
        ...
        scrollView.contentSize = CGSize(width: self.scrollView.frame.size.width * CGFloat(imagesCount), height: self.scrollView.frame.size.height)
        var xChod : CGFloat = 5
        
        for i in 0..<model.count {
           let productInfo = model[i]
            
            let imageView = UIImageView(frame: CGRect(x: xChod, y: 0, width: self.scrollView.frame.size.width/2-10, height: self.scrollView.frame.size.height-150))
            ...
            let nameLbl = UILabel(frame: CGRect(x: xChod, y: self.scrollView.frame.size.height-150, width: self.scrollView.frame.size.width/2-10, height: 50))
            ...
            let categoryLbl = UILabel(frame: CGRect(x: xChod, y: self.scrollView.frame.size.height-120, width: self.scrollView.frame.size.width/2-10, height: 50))
            ...
            let priceLbl = UILabel(frame: CGRect(x: xChod, y: self.scrollView.frame.size.height-90, width: self.scrollView.frame.size.width/2-10, height: 50))
            ...
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(ProductsScrollTableViewCell.imageViewtapped(tapGesture:)))
            imageView.addGestureRecognizer(tapGesture)
            
            xChod += self.scrollView.frame.size.width/2
        }
        ...
        timer = Timer.scheduledTimer(timeInterval: 3 , target: self, selector: #selector(ProductsScrollTableViewCell.loop), userInfo: nil, repeats: true)
        timer!.fire()
    }
